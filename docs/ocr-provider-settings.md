...
手書きモードのProvider、Parser、テスト資産は将来の再検証用に保持しているが、実サンプルでの認識精度を理由に、トップ画面では手書きメモの入力導線を提供していない。

再開する場合も、手書き文字の認識結果は自動確定せず、原画像との照合と人による補正を必須とする。
